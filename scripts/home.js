...
// Selecting all filter elements and buttons
const allFilterItems = document.querySelectorAll('.filter-item');
const allFilterBtns = document.querySelectorAll('.filter-btn');

// On page load, add the 'active-btn' class to the second button
window.addEventListener('DOMContentLoaded', () => {
    allFilterBtns[1].classList.add('active-btn');
});

// For each filter button, add event listeners for click
...
// Function to display filtered content
...
// Check if the document is in loading state
...
// Function executed when the add to cart button is clicked
...
// Format the time spent in a readable form
...
// Save the last login time on login
...
document.getElementById('logoutLink').addEventListener('click', function () {
    ...
    window.location.href = "https://blagojce-7.github.io/dropshipping/index.html"; // Redirect to the login page
});
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Display and hide the settings menu
    ...
    // Display the deactivate account modal
    document.getElementById('deactivateAccountLink').addEventListener('click', function () {
        const deactivateModal = document.getElementById('deactivateModal');
        const loggedInUser = JSON.parse(localStorage.getItem('loggedInUser'));
        document.getElementById('deactivationEmail').value = loggedInUser.email; // Auto-fill the email
        deactivateModal.style.display = 'block';
        document.body.classList.add('modal-open'); // Prevent scrolling of the body
    });

    // Close the deactivate account modal
    ...
    // Submit the deactivate account form
    ...
    function closeModal(modalId) {
        ...
        document.body.classList.remove('modal-open'); // Enable scrolling of the body
    }

    // Event listener for pressing Enter in the deactivate account modal
    document.addEventListener('keydown', function (event) {
        ...
        // Event listener for pressing Enter in the change password modal
    ...
    document.getElementById('showPasswordCheckbox').addEventListener('change', function () {
        ...
        if (this.checked) {
            currentPasswordInput.type = 'text'; // Show the password
            newPasswordInput.type = 'text'; // Show the password
        } else {
            currentPasswordInput.type = 'password'; // Hide the password
            newPasswordInput.type = 'password'; // Hide the password
        }
    });
});
